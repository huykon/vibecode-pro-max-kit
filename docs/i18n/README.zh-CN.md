...
your-project/
├── .claude/
│   ├── agents/              # 🤖 15 个专用 agent 定义
│   │   ├── vc-research-agent.md
│   │   ├── vc-execute-agent.md
│   │   └── ...
│   ├── skills/              # ⚡ 33 个自动发现的 skills
│   │   ├── vc-generate-plan/
│   │   ├── vc-security/
│   │   ├── vc-scout/
│   │   └── ...
...
这套 harness 给你的项目装上一整套开发体系——不只是一个 CLAUDE.md 文件，而是 **15 个专用 agent、33 个 skills**，加上一个阶段锁定的工作流，强制你的 agent **先理解再动手**。
...
agent 不只是读你的代码——它会**深度调研**学习别人怎么解决类似问题（`vc-docs-seeker` · `vc-scout`）。
...
在路由任何请求之前，扫描 <strong>33 个 skills</strong> 并匹配关键词。你说 "add webhook support"，<code>vc-security</code> + <code>vc-scenario</code> 自动浮现。
</td>
<td width="50%" valign="top">
<h1>💾</h1>
...
<code>vc-agent-strategy-compare</code> — 每个阶段边界自动推荐执行策略（顺序 / 并行 / 工作流 / agent 团队），含成本估算。
</td>
</tr>
<tr>
<td width="50%" valign="top">
